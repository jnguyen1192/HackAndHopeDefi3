...
class protoLoadPriorityDistrictListDB
{
    ...
    public $list_points_row = array(); // list points row as member
    public $list_areas_row = array(); // list areas row as member
        ...
        try
        {
            ...
            if ($res->num_rows > 0) {
                ...
                while($row = $res->fetch_assoc()) {
                    ...
                    array_push($this->list_points_row, $cols);
                }
    ...
    function select_areas_from_db($host="localhost", $port=3306, $db_name="proto_qp", $username="root", $password="root", $table="qp") // insert into db using credentials
    {
        // Create connection
        $conn = new mysqli($host, $username, $password, $db_name, $port); // use mysqsli function to connect into db
        // Check connection
        ...
        try
        {
            ...
            //$this->insert_PriorityDistrict($conn, $row); // insert into the database
            $sql = 'SELECT nom_epci, ST_Area(`geo_shape`) as polygon FROM `qp`'; // mysql query to select points from DB
            $res = $conn->query($sql);
            if ($res->num_rows > 0) {
                // output data of each row
                while($row = $res->fetch_assoc()) {
                    $cols = array("nom_epci" => $row["nom_epci"], "polygon" => $row["polygon"]);
                    //echo "nom_epci: " . $row["nom_epci"]. " - latitude longitude: " . $row["latitude"]. " " . $row["longitude"]. "<br>";
                    array_push($this->list_areas_row, $cols);
                }
                echo "Select awesome !!!"; // show a successfull message
                return 0; // return 0 in case it works
            } else {
